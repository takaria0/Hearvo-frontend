...
const GroupList = (props: GroupListProps) => {
  ...
  useEffect(() => {
    axios
      .get(`/groups?order_by=latest`, options)
      .then((res: any) => {
            ...
            <div style={{ paddingLeft: 2, wordWrap: "break-word" }}>
              ...
              {groupList.map((elem: any) => {
                ...
                return (
                  ...
                  >
                    <Link to={`/group/${elem.id}/feed`}>
                      <b>{elem.title}</b>
                    </Link>
                    <div>
                      <span>
                        {i18n.t("group.usersNum")} {elem.num_of_users}{" "}
                        {i18n.t("group.postsNum")} {elem.num_of_posts}
                        &nbsp;&nbsp;
                      </span>
                      <span style={{ float: "right", textAlign: "right" }}>
                        <div>
                          <button
                            onClick={(e) => copy(e, baseLink + elem.link)}
                          >
                            {maxWidth ? (
                              i18n.t("group.groupLinkCopy")
                            ) : (
                              <FileCopyIcon
                                style={{ padding: 0 }}
                              ></FileCopyIcon>
                          ...
                          &nbsp;&nbsp;
                          <button onSubmit={(e) => submit(e, elem.id)}>
                            {i18n.t("group.left")}
                          </button>
                        </div>
                      </span>
