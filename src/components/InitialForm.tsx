...
const InitialUserInfoForm = (props: any) => {
  ...
  const submit = (e: any) => {
    ...
    axios
      .put("/users?initial_setting=true", postObj, options)
      .then((res: any) => {
      ...
      <div>
        ...
        <div>
          <input
            ...
            maxLength={100}
            onChange={(e) => setMiddleName(e.target.value as string)}
          ></input>
        </div>
      </div>
    );
  ...
  const birthDayForm = () => {
    ...
    for (let year = currentYear - 120; year < currentYear; year++) {
      yearOption.push(<MenuItem value={year}>{year}</MenuItem>);
    }
    for (let month = 1; month <= 12; month++) {
      monthOption.push(<MenuItem value={month}>{month}</MenuItem>);
    }
...
const InitialTopicForm = (props: any) => {
  ...
  useEffect(() => {
    axios
      .get(`/topics?initial_topics=${JSON.stringify(INITIAL_TOPICS)}`, options)
      .then((res) => {
        const addChecked = res.data.map((elem: any) => {
  ...
  const change = (e: any, topic_id: number, topic: string, idx: number) => {
    switch (e.target.checked) {
      case false:
        // delete topic
        const newSaveTopicList = saveTopicList.filter(
          (elem: any) => elem.id !== topic_id
        );
      ...
      case true:
        ...
        setSaveTopicList([
          ...saveTopicList,
          { id: topic_id, topic: topic, checked: e.target.checked },
        ]);
    }
  };
  ...
  const submit = (e: any) => {
    ...
    axios
      .post("/topics/users", { topic_id_list }, options)
      .then((res: any) => {
        props.setFinish(true);
      })
      .catch((res: any) => {
      ...
      <Dialog open={true} fullScreen={fullScreen}>
        <form className={styles.initial_topic} onSubmit={(e) => submit(e)}>
          <List style={{ maxHeight: "75ch", overflow: "auto" }}>
            <h2 style={{ textAlign: "center", padding: "2ch 3.2ch" }}>
              {i18n.t("feed.chooseTopic")}
            </h2>
              ...
              return (
                <div>
                  ...
                  >
                    ...
                    />
                    <Checkbox
                      onChange={(e) => change(e, topic.id, topic.topic, idx)}
                      name={topic.topic}
                      value={topic.id}
...
const InitialForm = () => {
  ...
  if (isUserInfoFinished && !isTopicFormFinished) {
    return (
      <InitialTopicForm setFinish={setIsTopicFormFinished}></InitialTopicForm>
    );
  }

  return (
    <InitialUserInfoForm
      setFinish={setIsUserInfoFinished}
    ></InitialUserInfoForm>
  );
};
