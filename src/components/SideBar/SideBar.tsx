...
import {
  InvitePeopleViaTwitterButton,
  InvitePeopleViaTwitterButtonDetail,
} from "./InviteTwitter";
...
const DetailInfo = (props: any) => {
  return (
    ...
    >
      <small>
        <Link to="/tos">{i18n.t("sidebar.tos")}</Link>{" "}
        <Link to="/privacy">{i18n.t("sidebar.privacy")}</Link>
      </small>
    </div>
  );
};
...
const TrendingTopics = (props: any) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("/topics?sidebar=true", options)
      .then((res) => {
...
const RelatedPosts = (props: any) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/posts?related_post_id=${props.postId}`, options)
      .then((res) => {
  ...
  if (isLoading) {
    return (
      <div>
        <h3>{i18n.t("sidebar.relatedPosts")}</h3>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {relatedPosts.map((elem: any) => {
        const title =
          elem.title.length > 30 ? elem.title.slice(0, 30) + ".." : elem.title;
        return (
...
const GoogleAdSideBar = (props: any) => {
  ...
  useEffect(() => {
    if (window !== undefined) {
      setMedia(window.matchMedia("(min-width: 700px)"));
    }
    ...
    case "posts":
      ...
      return (
        <span>
          ...
          <InvitePeopleViaTwitterButtonDetail
            id={postId}
            text={i18n.t("twitterInvite.detailButtonText")}
          />
        </span>
      );
      break;
    ...
    default:
      return (
        <span>
          ...
          {/* <GoogleAdSideBar /> */}
          <InvitePeopleViaTwitterButton
            text={i18n.t("twitterInvite.buttonText")}
          />
        </span>
      );
      break;
