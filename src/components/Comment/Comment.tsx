...
const CommentItem = (props: any) => {
  ...
  try {
    myOwnCommentFavs = props.data.comment_favs.filter(
      (elem: any) => elem.user_info_id === props.userObj.id
    );
  } catch (error) {
    myOwnCommentFavs = [];
  }
  ...
  const likeSubmit = (data: any, value: number) => {
    ...
    axios
      .post("/comments/fav", { comment_id, good_or_bad }, options)
      .then((res) => {})
      .catch((err) => {});
  };
  ...
  const likeDelete = (data: any) => {
    ...
    axios
      .delete("/comments/fav", { data: { comment_id }, ...options })
      .then((res) => {})
      .catch((err) => {});
  };
  ...
  const goodStyle = isGoodClicked
    ? { margin: 0, padding: 0, fontSize: 12, color: "#004DBF" }
    : { margin: 0, padding: 0, fontSize: 12, color: "gray" };
  const badStyle = isBadClicked
    ? { margin: 0, padding: 0, fontSize: 12, color: "#004DBF" }
    : { margin: 0, padding: 0, fontSize: 12, color: "gray" };
    ...
    <span style={{ wordWrap: "break-word", whiteSpace: "pre-wrap" }}>
      ...
      <span>
        <span>
          &nbsp;&nbsp;
          <ThumbUpIcon
            onClick={(e) => onGoodClick(e)}
            style={goodStyle}
          ></ThumbUpIcon>
          &nbsp;{numOfGood}&nbsp;&nbsp;
          <ThumbDownIcon
            onClick={(e) => onBadClick(e)}
            style={badStyle}
          ></ThumbDownIcon>
          &nbsp;&nbsp;
        </span>
...
class Comment extends React.Component<CommentProps, CommentState> {
  ...
  updateData = () => {
    ...
    axios
      .get(`/comments?post_id=${this.props.postId}&order_by=popular`, options)
      .then((res: any) => {
  ...
  badSubmit = (e: any, data: any) => {
    ...
    axios
      .post("/comments/fav", { comment_id }, options)
      .then((res) => {})
      .catch((err) => {});
  };
  ...
  showName(userInfo: any) {
    return (
      ...
      <Link to={"/profile/" + userInfo.name + "/myposts"}>
          ...
          <li className={styles.com_li} key={props.id}>
            <div className={styles.body}>
              ...
              <span style={{ fontSize: "10px", textAlign: "right" }}>
                by {this.showName(props.user_info)},{" "}
                {this.getDiffTime(
                  props.created_at.slice(0, -7).replace("T", " ")
                )}
              </span>
              ...
              <ReplyComment
                ...
                handleParentPosted={this.handlePosted}
              ></ReplyComment>
            </div>
            ...
            <div className={styles.body}>
              ...
              <span style={{ textAlign: "right" }}>
                <Button
                  ...
                  onClick={(e) => this.click(e, props.id)}
                >
                  <ReplyIcon style={{ fontSize: 18 }} />
                  {i18n.t("eachPost.reply")}
                </Button>
              </span>
            </div>
          ...
          <div>
            <form onSubmit={(e) => this.baseSubmit(e)}>
              <div style={{ textAlign: "center" }}>
                <TextareaAutosize
                  maxLength={1000}
                  placeholder={i18n.t("eachPost.commentPlaceholder")}
                  rows={5}
                  className={styles.base}
                  onChange={(e) => this.baseChange(e)}
                  value={this.state.baseCommentContent}
                ></TextareaAutosize>
                <div className={styles.submit}>
                  <Button
                    ...
                    type="submit"
                    value="Submit"
                    disabled={!this.state.isCommentEntered}
                    className={
                      this.state.isCommentEntered
                        ? styles.comment_button
                        : styles.disabled_comment_button
                    }
                  >
                    {i18n.t("eachPost.commentButton")}
                  </Button>
                </div>
              </div>
            </form>
