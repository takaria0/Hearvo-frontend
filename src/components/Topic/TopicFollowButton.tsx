...
const TopicFollowButton = (props: any) => {
  ...
  useEffect(() => {
    axios
      .get(`/topics/users?topic_word=${props.topicWord}`, options)
      .then((res) => {
        setIsFollowed(false);
  ...
  const follow = (e: any) => {
    axios
      .post("/topics/users", { topic_id_list: [topicId] }, options)
      .then((res) => {
        setIsFollowed(true);
      })
      .catch((err) => {});
  ...
  if (isFollowed) {
    return (
      <span>
        &nbsp;&nbsp;&nbsp;
        <button style={unfollowButtonStyle} onClick={(e) => unfollow(e)}>
          {i18n.t("topicFollow.unfollow")}
        </button>
  ...
  return (
    <span>
      &nbsp;&nbsp;&nbsp;
      <button style={followButtonStyle} onClick={(e) => follow(e)}>
        {i18n.t("topicFollow.follow")}
      </button>
      {/* {resData.num_of_posts} | {resData.num_of_users} */}
    </span>
  );
