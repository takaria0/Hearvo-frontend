...
const CompareResult = (props: any) => {
  ...
  if (Array.isArray(props.data)) {
    initialSelectOption = props.data.map((elem: any) => {
      return { title: elem.title, value: elem.id, type: "post_id" };
    });
  } else {
    initialSelectOption = [
      { title: props.data.title, value: props.data.id, type: "post_id" },
    ];
  }

  initialSelectOption.push({
    title: i18n.t("compare.gender"),
    value: "gender",
    type: "gender",
  });
  initialSelectOption.push({
    title: i18n.t("compare.age"),
    value: "age",
    type: "age",
  });
  ...
  const change = (e: any, elem: any) => {
    switch (e.target.checked) {
      case true:
        ...
        setRawPostObj([
          ...rawPostObj,
          { title: elem.title, value: elem.value, type: elem.type },
        ]);
        break;
      ...
      case false:
        ...
        const deleteRawPostObj = rawPostObj.filter(
          (raw: any) => raw.value !== elem.value
        );
  ...
  const submit = (e: any) => {
    ...
    const first_target = {
      type: rawPostObj[0].type,
      data: rawPostObj[0].value,
    };
    const second_target = {
      type: rawPostObj[1].type,
      data: rawPostObj[1].value,
    };

    const postObj = {
      parent_id: initialSelectOption[0].value,
      first_target,
      second_target,
    };
    ...
    axios
      .post(`/vote_selects/compare`, postObj, options)
      .then((res) => {
  ...
  const barPlot = () => {
    ...
    const margin = mediaQueryMin900.matches
      ? { top: 50, right: 200, bottom: 50, left: 160 }
      : { top: 50, right: 150, bottom: 50, left: 100 };
    const keysVer1 = Object.keys(compareData.result[0])
      .map((key: string) => key)
      .slice(1);
    ...
    return (
      <div>
        ...
        <div style={{ height: 350 }}>
          <MyResponsiveBar
            data={JSON.parse(JSON.stringify(compareData.result))}
