...
const TopicHeader = (props: any) => {
  ...
  return (
    ...
    >
      <span>
        <b style={{ fontSize: "1.17em" }}>
          {i18n.t("feed.topic")} {props.topicTitle}
        </b>
        <TopicFollowButton topicWord={props.topicTitle}></TopicFollowButton>
        <div>
          {i18n.t("feed.posts")} {props.topicNumPosts}{" "}
          {i18n.t("feed.following")} {props.topicNumUsers}
        </div>
      </span>
    </div>
  );
...
class Feed extends React.Component<FeedProps, FeedState> {
  ...
  getData = (page: number) => {
    ...
    const keyword = keywordArray.includes("popular")
      ? "popular"
      : keywordArray.pop() || "";
    ...
    switch (feedType) {
      case "search":
        ...
        queryUrl = orderType
          ? `/posts?search=${searchWord}&type=${type}&page=${page}&keyword=${orderType}`
          : `/posts?search=${searchWord}&type=${type}&page=${page}`;
        axios
      ...
      case "topic":
        ...
        queryUrl = orderType
          ? `/posts?topic=${topicWord}&page=${newpage}&keyword=${orderType}`
          : `/posts?topic=${topicWord}&page=${newpage}`;
        axios
          .get(queryUrl, options)
          .then((res) => {
            this.setState({
              dataArray:
                page === 1
                  ? res.data.posts
                  : [...this.state.dataArray, ...res.data.posts],
              isLoaded: true,
              topicTitle: topicWord,
              topicNumPosts: res.data.topic.num_of_posts,
              topicNumUsers: res.data.topic.num_of_users,
            });
          })
          .catch((err) => {
      ...
      case "group":
        ...
        queryUrl = orderType
          ? `/posts?group_id=${groupId}&page=${newpage}&keyword=${orderType}`
          : `/posts?group_id=${groupId}&page=${newpage}`;
        axios
          .get(`/groups?id=${groupId}`, options)
          .then((res) => {
            this.setState({
              groupTitle: res.data.already_joined
                ? res.data.title
                : i18n.t("feed.groupDoesntExist"),
            });
      ...
      case "popular":
        axios
          .get(`/posts?keyword=popular&page=${newpage}&time=${time}`, options)
          .then((res) => {
            this.setState({
              dataArray:
                page === 1 ? res.data : [...this.state.dataArray, ...res.data],
              isLoaded: true,
            });
          })
          .catch((err) => {
      ...
      case "latest":
        axios
          .get(`/posts?keyword=latest&page=${newpage}`, options)
          .then((res) => {
            this.setState({
              dataArray:
                page === 1 ? res.data : [...this.state.dataArray, ...res.data],
  ...
  componentDidUpdate = (prevProps: any, prevState: any) => {
    ...
    if (
      prevProps.isPosted !== this.props.isPosted ||
      this.props.isPosted === true
    ) {
      this.getData(1);
  ...
  feedList = () => {
    const feedArray = this.state.dataArray.map((data: any, idx: number) => {
      return (
        <EachPost
          post_detail_id={""}
        ...
        <div>
          {skeletonList.map((elem: any) => (
            <SkeletonTheme color="#FFFFFF" highlightColor="#E2E2E2">
              <p style={{ border: "solid", borderWidth: 1, borderRadius: 6 }}>
                <Skeleton duration={0.8} height={250} />
              </p>
            </SkeletonTheme>
          ))}
        </div>
        ...
        <div>
          <div>
            ...
            <TopicHeader
              ...
              topicNumUsers={this.state.topicNumUsers}
              topicNumPosts={this.state.topicNumPosts}
            ></TopicHeader>
            {this.state.groupTitle ? (
              <h3>
                {i18n.t("feed.group")} {this.state.groupTitle}
              </h3>
            ) : (
              ""
            )}
            {this.state.searchWord ? (
              <h3>
                {this.state.searchWord} {i18n.t("feed.searchResult")}
              </h3>
            ) : (
              ""
            )}
          ...
          <ul className={styles.ul}>
            {this.state.dataArray.length > 0 ? (
              ...
              <div>
                <div>{i18n.t("feed.noContent")}</div>
                <div>{i18n.t("feed.followMore")}</div>
              </div>
            )}
          </ul>
