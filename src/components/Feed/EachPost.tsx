...
const VotingLengthSelector = (props: any) => {
  return (
    <div style={{}}>
      ...
      <select
        className={poststyles.vote_length_selector}
        size={1}
        onChange={(e) => {
          props.changeEndAt(e.target.value);
        }}
      >
...
const PollRecord = (props: any) => {
  const initPostDetailObj =
    props.postDetailType === "target"
      ? props.data.target_post_detail
      : props.data.current_post_detail;
  const [postDetailObj, setPostDetailObj] = useState<any>(initPostDetailObj);

  useEffect(() => {}, [postDetailObj]);
  ...
  return (
    <div style={style}>
      <div>
        {i18n.t("pollRecord.votingLength")}{" "}
        {postDetailObj.start_at.slice(0, 10)} ~{" "}
        {postDetailObj.end_at.slice(0, 10)}
        <span style={recreatePollStyle}>
          <RecreatePollButton
            data={props.data}
            postDetailType={props.postDetailType}
          />
        </span>
      </div>
      {props.data.post_details.length > 1 ? (
        <span>
          {i18n.t("pollRecord.pastPolls")}{" "}
          {props.data.post_details.map((each: any, idx: number) => {
            return (
              <span>
                <Link
                  style={linkStyle}
                  to={`/posts/${props.data.id}/record/${each.id}`}
                >
                  {idx + 1}
                </Link>{" "}
                |{" "}
...
const RecreatePollButton = (props: any) => {
  ...
  const submit = (e: any) => {
    ...
    const end_at = new Date(
      currentDate.setHours(currentDate.getHours() + endAtPlus)
    )
      .toISOString()
      .slice(0, -8);
    const url = "/posts";
    ...
    <span>
      <Dialog open={modal}>
        <div style={closeStyle}>
          <Button onClick={() => onClose()} disableRipple disableElevation>
            <CloseIcon />
          </Button>
        </div>
        <div style={dialogStyle}>
          ...
          <div style={titleStyle}>
            <VotingLengthSelector changeEndAt={setEndAtPlus} />
          </div>
        </div>
        <div>
          <div style={titleStyle}>
            ...
            <Button
              ...
              disableRipple
              disableElevation
              className={styles.recreate_poll_button}
            >
              {i18n.t("pollRecord.start")}
            </Button>
          </div>
        </div>
      </Dialog>
    ...
    default:
      const content =
        props.data.content.length > 200
          ? props.data.content.slice(0, 200) + "..."
          : props.data.content;
      return (
        <div>
          <div style={{ marginTop: 10, marginBottom: 10, marginRight: 10 }}>
            <div className={styles.title}>
              <Link
                ...
                className={styles.each_post_link}
              >
                {props.data.title}
              </Link>
            </div>
...
const PollUserList = (props: any) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/users?post_detail_id=${props.post_detail_id}`, options)
      .then((res) => {
      ...
      >
        ...
        {isLoading
          ...
          : userList.map((user: any) => {
              return (
                <MenuItem>
                  <Link to={"/profile/" + user.name + "/myposts"}>
                    <MenuTitle>{user.profile_name}</MenuTitle>
                    <MenuBio>{user.description}</MenuBio>
                  </Link>
                </MenuItem>
              );
            })}
    ...
    <div>
      ...
      >
        ...
        <span>
          <TwitterShareButton
            title={
              i18n.t("twitterInvite.detailText") +
              " | Hearvo | " +
              props.data.title
            }
            url={
              "https://" + window.location.hostname + "/posts/" + props.data.id
            }
          >
            <TwitterIcon />
          </TwitterShareButton>
...
const getDiffTime = (datetime: string) => {
  ...
  const diff = moment(moment(moment().format("YYYY-MM-DD HH:mm:ss"))).diff(
    datetime
  );
...
const getEndTime = (datetime: string) => {
  const diff = moment(datetime).diff(
      ...
      if (str.startsWith("#")) {
        return (
          <Link
            ...
            to={`/search?q=${str.replace("#", "")}&type=hash_tag`}
          >
            {str}
          </Link>
        );
    ...
    >
      <tr>
        <th style={{ border: "1px solid black", borderCollapse: "collapse" }}>
          {" "}
          {i18n.t("eachPost.candidate")}{" "}
        </th>
        {baseData.mj_options.map((obj: any) => {
          return (
            <th
        ...
        return (
          <tr>
            ...
            {baseData.mj_options.map((obj: any) => {
              ...
              const mj_option_count = countObj.filter((el: any) => {
                return el.mj_option_id === mj_option_id;
              });
              return (
                <td
                  style={{
                ...
                >
                  {mj_option_count.length > 0 ? mj_option_count[0].count : 0}
                </td>
              );
            })}
          </tr>
...
class EachPost extends React.Component<EachPostProps, EachPostState> {
  ...
  render() {
    const currentFirstURL =
      window.location.pathname.split("/").length > 1
        ? window.location.pathname.split("/")[1]
        : "";
    const postDetailObj =
      this.state.postDetailType === "target"
        ? this.props.data.target_post_detail
        : this.props.data.current_post_detail;
      ...
      case 1:
        ...
        if (currentFirstURL === "posts") {
          return (
            <li className={styles.li} key={this.props.data.id}>
              <PostHeader
                link={currentFirstURL}
                data={this.props.data}
                postDetailType={this.state.postDetailType}
              ></PostHeader>
              <PollRecord
                post_detail_id={this.props.post_detail_id}
                data={this.props.data}
                postDetailType={this.state.postDetailType}
              />
              ...
              <div className={styles.vote_section}>
                <VoteSelect
                  hasVoted={this.props.data.already_voted}
                  periodEnd={this.props.data.vote_period_end}
                  isLogin={this.props.isLogin}
                  voteContent={postDetailObj.vote_selects}
