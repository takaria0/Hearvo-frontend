...
class EachVoteMj extends React.Component<EachVoteMjProps, EachVoteMjState> {
  ...
  submit = (e: any) => {
    ...
    if (this.props.isLogin === false) {
      ...
      this.props.history.push(
        "/login" + "?destination=" + "posts" + "&value=" + post_id
      ); // ログイン後、投稿詳細ページに飛ぶ
      return;
    }
    ...
    axios
      .post("/vote_mj_users", voteMjPostObj, config)
      .then((res) => {
        const countVotePostObj = { post_id: this.props.postId };
        axios.post("/count_vote_mjs", countVotePostObj, config).then((res) => {
          this.setState({
            data: res.data,
  ...
  change(e: any, voteMjId: number) {
    const filteredArray = this.state.voteMjCount.filter((el: any) => {
      return el.vote_mj_id != voteMjId;
    });
    filteredArray.push({
      vote_mj_id: voteMjId,
  ...
  render() {
    ...
    if (
      (this.state.isClicked || this.props.hasVoted === true) &&
          ...
          <div className={styles.vote_section}>
            ...
            >
              <tr>
                ...
                {this.state.data.mj_options.map((obj: any) => {
                  return (
                    <th
                      style={{
                        border: "1px solid black",
                  ...
                  <tr>
                    ...
                    {this.state.data.mj_options.map((obj: any) => {
                      ...
                      return (
                        ...
                        >
                          {mj_option_count.length > 0
                            ? mj_option_count[0].count
                            : 0}
                        </td>
                      );
                    })}
                ...
                return (
                  <div>
                    ...
                    <div
                      ...
                      onChange={(e) => this.change(e, voteMjId)}
                    >
