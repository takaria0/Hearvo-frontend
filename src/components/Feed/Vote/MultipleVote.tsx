...
const MultipleVote = (props: any) => {
  ...
  useEffect(() => {
    ...
    if (props.postDetailType === "target") {
      url = `/posts?parent_id=${props.postId}&post_detail_id=${props.post_detail_id}`;
    }
    let options = {};
    if (!jwt) {
  ...
  const startClick = (e: any) => {
    ...
    if (props.isLogin === false) {
      history.push(
        "/login" + "?destination=" + "posts" + "&value=" + props.postId
      ); // ログイン後、投稿詳細ページに飛ぶ
      return;
    }
  ...
  const addResult = (e: any, vote_select_id: number, post_id: number) => {
    ...
    if (data.length - 1 === dataIdx) {
      const voteObj = {
        parent_id: props.postId,
        result: [...voteResult, { vote_select_id, post_id }],
      };
      let options = {};
      if (!jwt) {
        options = { headers: { Country: process.env.REACT_APP_COUNTRY } };
      ...
      axios
        .post("/multiple_vote_users", voteObj, options)
        .then((res) => {
    ...
    return (
      <div className={styles.content}>
        <div style={{ padding: 10 }}>
          <h2>
            {i18n.t("newPost.continuasVote")} {dataIdx + 1}
          </h2>
            ...
            return (
              <div>
                <div
                  ...
                  onClick={(e) => addResult(e, each.id, currentData.id)}
                >
                  {each.content}
                </div>
              </div>
    ...
    const resultJSX = data.map((each: any, idx: number) => {
      const postDetailObj =
        props.postDetailType === "target"
          ? each.target_post_detail
          : each.current_post_detail;
      let x;
      let y;
      let plotData;
      let layout;
      ...
      x = postDetailObj.vote_selects.map((da: any) => {
        return (da.count * 100) / totalVote;
      });
      y = postDetailObj.vote_selects.map((da: any) => {
        return da.content;
      });

      const voteIdList = postDetailObj.vote_selects.map((da: any) => {
        return da.vote_select_id;
      });
      plotData = [
        {
          type: "bar",
      ...
      layout = {
        title: `${i18n.t("eachPost.totalVote")}: ${totalVote}`,
        ...
        return (
          <div>
            <h2>
              ...
              <ClearIcon style={{ marginBottom: -5, padding: 0 }} />{" "}
              {data.length}
            </h2>
            <Link
    ...
    return (
      <div>
        <h2>
          ...
          ></InsertDriveFileIcon>
          <ClearIcon style={{ marginBottom: -5, padding: 0 }}></ClearIcon>
          {data.length}
        </h2>
        <Button
          style={{
