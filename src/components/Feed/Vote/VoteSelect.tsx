...
const VoteSelect = (props: any) => {
  ...
  const callApi = (voteSelectPostObj: any, config: any) => {
    ...
    axios
      .post("/vote_select_users", voteSelectPostObj, config)
      .then((res) => {
        Mixpanel.track("Successful Vote", { ...voteSelectPostObj });
        axios.get(`/posts?id=${data.id}`, config).then((res) => {
          setData(res.data);
          setIsLoading(false);
          setHasVoted(true);
        });
  ...
  if (periodEnd || hasVoted) {
    ...
    const x = postDetailObj.vote_selects.map((da: any) => {
      return (da.count * 100) / postDetailObj.num_vote;
    });
    const y = postDetailObj.vote_selects.map((da: any) => {
      return da.content;
    });
    ...
    const plotData = [
      {
        ...
        y: y,
        orientation: "h",
        myVote: props.data.my_vote,
        voteIdList: voteIdList,
      },
    ];
    const layout = {
      title: `${i18n.t("eachPost.totalVote")}: ${postDetailObj.num_vote}`,
    ...
    if (currentFirstURL === "posts") {
      return (
        <div className={styles.vote_section}>
          ...
          <AttributePlotPie
            ageDist={props.data.age_distribution}
            genderDist={props.data.gender_distribution}
          />
        </div>
      );
    }
